				...
				{#if visibleTabs.length > 0}
					{#each visibleTabs as tabId (tabId)}
						...
						{:else if tabId === 'connections'}
							{#if $user?.role === 'super_admin' || ($user?.role === 'user' && $config?.features?.enable_direct_connections)}
								<button
									class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left transition {selectedTab ===
									'connections'
										? ''
							...
							{/if}
						{:else if tabId === 'tools'}
							{#if $user?.role === 'super_admin' || ($user?.role === 'user' && $user?.permissions?.features?.direct_tool_servers)}
								<button
									class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left transition {selectedTab ===
									'tools'
										? ''
